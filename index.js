...
bot.on(':users_shared', async (ctx) => {
    ...
    // console.log('id приглашаемого пользоваателя: ', newUserId);
    const sponsorId = ctx.message.user_shared.request_id
    // console.log('id приглашающего пользоваателя: ', sponsorId);
	try {
		...
		if (pass.status == 'kicked') {
			...
			if (!Object.hasOwn(db.data.users, sponsorId)) {
				await db.update(({ users }) => {
                    users[sponsorId] = [];
                    // console.log('235 строка: Добавляем в БД спонсора: ', sponsorId);
                    users[sponsorId].push(newUserId)
                    // console.log('237 строка: Добавляем в БД newUser: ', newUserId);
				})
                ...
                // console.log(`242 строка: Пользователь ${newUserId} уже был добавлен в БД`);
				...
				// console.log(`246 строка: добавляем ${newUserId} в БД`)
				await ctx.reply(`🔗 Отправьте ссылку на канал другу и попросите подписаться: ${channelLink}`)
				await ctx.reply('⏱ Как только ваш друг подпишется на канал, вам добавится билет розыгрыша ')
			}
		}
	} catch (error) {
		// console.log(`252 строка: CATCH Пользователь ${newUserId} не открывал бота ранее`);
		await db.read()
		if (!Object.hasOwn(db.data.users, sponsorId)) {
			await db.update(({ users }) => {
                users[sponsorId] = [];
                // console.log('257 строка: Добавляем в БД спонсора: ', sponsorId);
                users[sponsorId].push(newUserId)
                // console.log('259 строка: Добавляем в БД newUser: ', newUserId);
			})
			await ctx.reply(`🔗 Отправьте ссылку на канал другу и попросите подписаться: ${channelLink}`)
			await ctx.reply('⏱ Как только ваш друг подпишется на канал, вам добавится билет розыгрыша ')
		} else if (db.data.users[sponsorId].includes(newUserId)) {
            // console.log(`264 строка: CATCH Пользователь ${newUserId} уже был добавлен в БД`);
			await ctx.reply('❗⚠️ Вы уже приглашали этого пользователя\nПопробуйте пригласить кого-нибудь другого')
		} else {
			await db.update(({ users }) => users[sponsorId].push(newUserId))
            // console.log(`268 строка: CATCH добавляем ${newUserId} в БД`)
			await ctx.reply(`🔗 Отправьте ссылку на канал другу и попросите подписаться: ${channelLink}`)
			await ctx.reply('⏱ Как только ваш друг подпишется на канал, вам добавится билет розыгрыша ')
		}
	}
